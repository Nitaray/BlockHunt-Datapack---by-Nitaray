team remove Seeker

team remove Hider

team remove Spectator

team remove Lobby



scoreboard objectives remove Health

scoreboard objectives remove Timer

scoreboard objectives remove Ticks

scoreboard objectives remove GameState

scoreboard objectives remove BlockType

scoreboard objectives remove mx

scoreboard objectives remove my

scoreboard objectives remove mz

scoreboard objectives remove tmp

scoreboard objectives remove TNT

scoreboard objectives remove HayBale

scoreboard objectives remove Anvil

scoreboard objectives remove CraftingTable

scoreboard objectives remove Furnace

scoreboard objectives remove FlowerPot

scoreboard objectives remove NoteBlock

scoreboard objectives remove Melon



gamemode creative @a

execute at @e[tag=Map] run fill ~-2 ~5 ~-2 ~2 ~15 ~2 minecraft:air

kill @e[tag=BlockHunt]